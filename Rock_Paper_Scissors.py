...
class Rock_Paper_Scissor:
    ...
    def createWindow(self):
        ...
        self.name_Frm = tkin.Frame(self.root, bg="sky blue")
        self.name_Frm.pack(pady=20)

        gameLabel=tkin.Label(self.name_Frm, text="ROCK-PAPER-SCISSORS", font=("Eras Bold Itc", 14), bg="sky blue")
        gameLabel.grid(row=0, column=0, columnspan=2, pady=10)
        
        player1Label=tkin.Label(self.name_Frm, text="Player 1 Name:", font=("Times New Roman", 12), bg="sky blue")
        player1Label.grid(row=1, column=0, pady=10)
        self.player1data = tkin.Entry(self.name_Frm)
        self.player1data.grid(row=1, column=1)

        player2Label=tkin.Label(self.name_Frm, text="Player 2 Name:", font=("Times New Roman", 12), bg="sky blue")
        player2Label.grid(row=2, column=0, pady=10)
        self.player2data = tkin.Entry(self.name_Frm)
        self.player2data.grid(row=2, column=1)

        startGameBtn=tkin.Button(self.name_Frm, text="Start Game", command=self.startGame, font=("Times New Roman",11), bg="orange")
    ...
    def setupGame(self):
        self.score_frame = tkin.Frame(self.root, bg="sky blue")
        self.score_frame.pack(pady=10)

        self.player1_label = tkin.Label(self.score_frame, text=f"{self.player1}: 0", bg="sky blue", font=("Arial", 12))
        self.player1_label.grid(row=0, column=0, padx=20)

        self.player2_label = tkin.Label(self.score_frame, text=f"{self.player2}: 0", bg="sky blue", font=("Arial", 12))
        self.player2_label.grid(row=0, column=1, padx=20)

        self.gameFrm = tkin.Frame(self.root, bg="sky blue")
        self.gameFrm.pack(pady=20)

        self.player1Btn = tkin.Button(self.gameFrm, text="Play Player 1", command=self.playPlayer1Btn, bg="orange")
        self.player1Btn.grid(row=1, column=0, padx=20)

        self.player2Btn = tkin.Button(self.gameFrm, text="Play Player 2", command=self.playPlayer2Btn, bg="orange")
        self.player2Btn.grid(row=1, column=1, padx=20)

        self.resultBoard = tkin.Label(self.root, text="", bg="purple", font=("Arial", 14))
